...
run of the file system. Uses the hash sytem to detect changes in files, as well as new or missing files 
from the last run through. This mimics Crowdstrike Falcon at a much smaller scale.

# What it currently does
Performs baseline scanning useing SHA-256 hashing. Diff mode compares the previously saved baseline
...
file. Subsequent runs use the previously created baseline to run a diff mode - reporting modifed, added, and
removed files.
